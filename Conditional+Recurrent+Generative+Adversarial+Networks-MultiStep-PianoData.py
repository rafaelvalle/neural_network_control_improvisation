...
print "d_batch_size", d_batch_size
min_len = 32
max_len = 64
...
def build_discriminator(params):
    # input layers
    l_in = lasagne.layers.InputLayer(shape=d_specs['input_shape'],
                                     name='d_in')
    l_mask = lasagne.layers.InputLayer(shape=d_specs['mask_shape'],
                                       name='d_mask')
...
def build_generator_lstm(params):
    ...
    l_in = lasagne.layers.InputLayer(
        shape=g_specs['input_shape'], input_var=g_in_D, name='g_in')
    l_noise = lasagne.layers.InputLayer(
        shape=g_specs['noise_shape'], input_var=g_in_Z, name='g_noise')
    l_cond = lasagne.layers.InputLayer(
        shape=g_specs['cond_shape'], input_var=g_in_C, name='g_cond')
    l_mask = lasagne.layers.InputLayer(
        shape=g_specs['mask_shape'], input_var=g_in_M, name='g_mask')
    ...
    l_forward_data = lasagne.layers.recurrent.LSTMLayer(
        ...
        learn_init=True, grad_clipping=g_specs['grad_clip'],
    ...
    l_out = lasagne.layers.DenseLayer(
        l_concat, num_units=g_specs['n_output_units'], num_leading_axes=2)
    ...
    l_out = lasagne.layers.ReshapeLayer(l_out_flat, g_specs['output_shape'])
    """
