...
def DCGANGenerator(n_samples, inputs, noise=None, dim=DIM/2, bn=True,
    ...
    # 64 notes and 32 timesteps
    prefix = tf.reshape(inputs, [-1, N_CHANNELS, 64, 32])
    print("prefix shape {}".format(prefix.shape))

    # discriminator
    d_output = lib.ops.conv2d.Conv2D(
        'G_Discriminator.1', N_CHANNELS, dim, 5, prefix, stride=2)
    d_output_1 = nonlinearity(d_output)
    print("d_output_1 shape {}".format(d_output_1.shape))

    d_output = lib.ops.conv2d.Conv2D(
        'G_Discriminator.2', dim, 2*dim, 5, d_output_1, stride=2)
    if bn:
        d_output = Batchnorm('G_Discriminator.BN2', [0,2,3], d_output)
    d_output_2 = nonlinearity(d_output)
    print("d_output_2 shape {}".format(d_output_2.shape))

    d_output = lib.ops.conv2d.Conv2D(
        'G_Discriminator.3', 2*dim, 4*dim, 5, d_output_2, stride=2)
    if bn:
        d_output = Batchnorm('G_Discriminator.BN3', [0,2,3], d_output)
    d_output_3 = nonlinearity(d_output)
    print("d_output_3 shape {}".format(d_output_3.shape))

    d_output = lib.ops.conv2d.Conv2D(
        'G_Discriminator.4', 4*dim, 8*dim, 5, d_output_3, stride=2)
    if bn:
        d_output = Batchnorm('G_Discriminator.BN4', [0,2,3], d_output)
    d_output_4 = nonlinearity(d_output)
    print("d_output_4 shape {}\n".format(d_output_4.shape))
    # d_output = tf.reshape(output, [-1, 4*4*8*dim])
    ...
    output = tf.reshape(output, [-1, 8*dim, 4, 2])
    print("g_output_1 shape {}".format(output.shape))
    if bn:
        output = Batchnorm('Generator.BN1', [0,2,3], output)
    output = nonlinearity(output)
    output = tf.concat([output, d_output_4], 3)
    print("concat 1 shape {}".format(output.shape))

    output = lib.ops.deconv2d.Deconv2D('Generator.2', 8*dim, 4*dim/2, 5, output)
    print("g_output_2 shape {}".format(output.shape))
    if bn:
        output = Batchnorm('Generator.BN2', [0,2,3], output)
    output = nonlinearity(output)
    output = tf.reshape(output, [-1, 4*dim, 8, 4])
    output = tf.concat([output, d_output_3], 3)
    print("concat 2 shape {}".format(output.shape))

    output = lib.ops.deconv2d.Deconv2D('Generator.3', 4*dim, 2*dim/2, 5, output)
    print("g_output_3 shape {}".format(output.shape))
    if bn:
        output = Batchnorm('Generator.BN3', [0,2,3], output)
    output = nonlinearity(output)
    output = tf.reshape(output, [-1, 2*dim, 16, 8])
    output = tf.concat([output, d_output_2], 3)
    print("concat 3 shape {}".format(output.shape))

    output = lib.ops.deconv2d.Deconv2D('Generator.4', 2*dim, dim/2, 5, output)
    print("g_output_4 shape {}".format(output.shape))
    if bn:
        output = Batchnorm('Generator.BN4', [0,2,3], output)
    output = nonlinearity(output)
    output = tf.reshape(output, [-1, dim, 32, 16])
    output = tf.concat([output, d_output_1], 3)
    print("concat 4 shape {}".format(output.shape))

    output = lib.ops.deconv2d.Deconv2D('Generator.5', dim, N_CHANNELS, 5, output)
    output = tf.tanh(output)
    output = tf.reshape(output, [-1, 1, 64, 32])
    print("g_output_5 shape {}".format(output.shape))

    #output = tf.concat([inputs, output], 3)
    #print("comb output shape {}".format(output.shape))

    lib.ops.conv2d.unset_weights_stdev()
    lib.ops.deconv2d.unset_weights_stdev()
    lib.ops.linear.unset_weights_stdev()
    output = tf.reshape(output, [-1, OUTPUT_DIM])
    print("output flat shape {}".format(output.shape))
...
with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as session:
    ...
    for device_index, (device, real_data_conv) in enumerate(zip(DEVICES, split_real_data_conv)):
        with tf.device(device):
            reg_noise = tf.random_normal(
                    ...
                    mean=0., stddev=0.001)
            real_data_patch = real_data_conv[:, :, :, :32]
            print("real_data_path shape {}".format(real_data_patch.shape))
            real_data = tf.reshape(real_data_conv, [BATCH_SIZE/len(DEVICES), OUTPUT_DIM])
            real_data += reg_noise
            fake_data = Generator(BATCH_SIZE/len(DEVICES), real_data_patch)
    ...
    if MODE == 'wgan':
        ...
        disc_train_op = tf.train.AdamOptimizer(
            learning_rate=WGAN_GP_CLR, beta1=0.5, beta2=0.9).minimize(
                disc_cost, var_list=lib.params_with_name('Discriminator.'),
                colocate_gradients_with_ops=True)
    elif MODE == 'dcgan':
        gen_train_op = tf.train.AdamOptimizer(
            learning_rate=2e-4, beta1=0.5).minimize(
                gen_cost, var_list=lib.params_with_name('Generator'),
                colocate_gradients_with_ops=True)
        disc_train_op = tf.train.AdamOptimizer(
            learning_rate=2e-4, beta1=0.5).minimize(
        ...
        gen_train_op = tf.train.RMSPropOptimizer(learning_rate=1e-4).minimize(
            gen_cost, var_list=lib.params_with_name('Generator'),
            colocate_gradients_with_ops=True)
        disc_train_op = tf.train.RMSPropOptimizer(learning_rate=1e-4).minimize(
            disc_cost, var_list=lib.params_with_name('Discriminator.'),
            colocate_gradients_with_ops=True)
    else:
        raise Exception()
    ...
    for device_index, device in enumerate(DEVICES):
        ...
        all_fixed_noise_samples.append(Generator(n_samples, real_data_patch, noise=fixed_noise[device_index*n_samples:(device_index+1)*n_samples]))
    if tf.__version__.startswith('1.'):
    ...
    def generate_image(iteration):
        samples = session.run(all_fixed_noise_samples, feed_dict={real_data_conv: _x})
    ...
    for iteration in range(0, ITERS):
        ...
        for i in range(disc_iters):
            _data, _ = train_gen.next()
            _data = _data.reshape((BATCH_SIZE, N_CHANNELS, alphabet_size, i_len))
            _disc_cost, _, _fake_data = session.run([disc_cost, disc_train_op, fake_data], feed_dict={all_real_data_conv: _data})
            if MODE == 'wgan':
